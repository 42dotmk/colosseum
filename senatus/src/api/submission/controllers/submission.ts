  ...
  ({ strapi }) => ({
    async submit(ctx) {
      try {
        ...
        const { publish, disconnect } = await connect(
          strapi.config.get("server.app.rabbitUrl"),
        );
        ...
        ctx.body = {
          state: "Queued",
          executions: executionsToQueue.map(
            (pair) => pair.execution.documentId,
          ),
        };
      } catch (err) {
        console.log(err);
    ...
    async getExecutions(ctx) {
      try {
        ...
        const results = executions.map((exec) => {
          if (!exec.testCase) {
            throw new Error(
              `Execution ${exec.documentId} has no associated test case`,
            );
          }
